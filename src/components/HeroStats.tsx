  ...
  return (
    <section className="relative pt-6 pb-8">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Hero Header Banner */}
        <div className="glass-panel-glow rounded-2xl p-6 sm:p-8 mb-8 border border-slate-700/80 bg-gradient-to-r from-slate-900/95 via-slate-900/85 to-[#0B1528]/90">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="max-w-3xl">
              <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-teal-500/15 border border-teal-500/40 text-teal-300 text-xs font-medium mb-3">
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-3">
              ...
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center space-x-2 px-4 py-3 rounded-xl bg-slate-800/90 hover:bg-slate-700/80 border border-slate-700 text-slate-200 hover:text-white font-medium text-sm transition-all cursor-pointer whitespace-nowrap"
              >
          ...
          {metricCards.map((card) => {
            ...
            return (
              <div
                key={card.id}
                id={card.id}
                className={`relative rounded-xl glass-panel p-5 border ${card.borderColor} bg-gradient-to-b ${card.bgGlow} transition-all duration-200 group`}
              >
                {card.highlightPulse && (
                ...
                <div className="mt-2.5 flex items-center text-xs space-x-1.5 pt-2 border-t border-slate-800/70">
                  <span
                    className={`font-semibold font-mono flex items-center ${
                      card.isPositive ? "text-teal-400" : "text-rose-400"
                    }`}
                  >
                    {card.delta}
                  </span>
                  <span className="text-slate-400 truncate">{card.deltaLabel}</span>
                </div>
              </div>
            );
          })}
